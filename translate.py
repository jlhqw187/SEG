...
while times:
    ...
    translate_range = 10
    translate_x = np.random.randint(-translate_range, translate_range)
    translate_y = np.random.randint(-translate_range, translate_range)
    ...
    for label_info in label_data['shapes']:
        ...
        translated_points = [(point[0] + translate_x, point[1] + translate_y) for point in points[0]]
        translated_points = np.array(translated_points, np.int32).reshape((-1, 1, 2))
        ...
        cv2.drawContours(edge_mask, translated_points, -1, (255, 255, 255), line_width)

        edge2inpaint.append(edge_mask)
        cv2.drawContours(object_mask, translated_points, -1, (255, 255, 255), cv2.FILLED)
